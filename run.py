...
def parse_args():
    ...
    parser.add_argument('-l', '--level', action='store_true', help='uses non-default level')
    parser.add_argument('-t', '--target', action='store_true', help='uses non-default target')
...
def main():
    ...
    if args.gather:
        ...
        for (idx, line) in enumerate(lines, 1):
            ...
            mapping = {'S': 'sets.py', 'D': 'dfs.py', 'H': 'hail.py', 'L': 'level.py', 'T': 'target.py'}
            program = mapping.get(method, '')
            fname = "%s.in" % str(idx).zfill(4)
            args = ["python", program, "-f", fname, "-s", seed]
            print("Running %s" % args)
            subprocess.check_call(args)
    else:
        program = "dfs.py" if args.dfs else "target.py" if args.target else \
                  'level.py' if args.level else 'sets.py'
